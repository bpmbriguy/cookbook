...
class Users_social extends CI_Model
{
	...
	function create_facebook_connect( $user_id, $fb_id, $token )
	{
        ...
        function user_image( $user_id, $size = 'normal' )
        {
            $this->db->select('facebook_id')->where('user_id', $user_id);
            
            $query = $this->db->get($this->profile_table_name);
            
            if ($query->num_rows() == 1) 
            {
                $fb_id = $query->row()->facebook_id;
                $url = "https://graph.facebook.com/$fb_id/picture?type=$size";
                return $url;
            }
        }
